...
function useMutationLoading(source, enabled) {
  if (!enabled) {
    return source.pipe((0, import_operators.map)((result) => __spreadProps(__spreadValues({}, result), {
      loading: false
    })));
  }
  return source.pipe((0, import_operators.startWith)({
    loading: true
  }), (0, import_operators.map)((result) => __spreadProps(__spreadValues({}, result), {
...
var ZoneScheduler = class {
  ...
  schedule(work, delay = 0, state) {
    return this.zone.run(() => import_rxjs.queueScheduler.schedule(work, delay, state));
  }
};
function fixObservable(obs) {
  obs[import_rxjs.observable] = () => obs;
  return obs;
}
function wrapWithZone(obs, ngZone) {
  return obs.pipe((0, import_operators.observeOn)(new ZoneScheduler(ngZone)));
}
function useInitialLoading(obsQuery) {
  return function useInitialLoadingOperator(source) {
    return new import_rxjs.Observable(function useInitialLoadingSubscription(subscriber) {
...
var QueryRef = class {
  ...
  constructor(obsQuery, ngZone, options) {
    ...
    const wrapped = wrapWithZone((0, import_rxjs.from)(fixObservable(this.obsQuery)), ngZone);
...
var ApolloBase = class {
  ...
  watchFragment(options, extra) {
    const obs = (0, import_rxjs.from)(fixObservable(this.ensureClient().watchFragment(__spreadValues({}, options))));
    return extra && extra.useZone !== true ? obs : wrapWithZone(obs, this.ngZone);
  }
  subscribe(options, extra) {
    const obs = (0, import_rxjs.from)(fixObservable(this.ensureClient().subscribe(__spreadValues({}, options))));
    return extra && extra.useZone !== true ? obs : wrapWithZone(obs, this.ngZone);
  }
  /**
...
var ApolloModule = class _ApolloModule {
  static ɵfac = function ApolloModule_Factory(__ngFactoryType__) {
